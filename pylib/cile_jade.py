...
"""A Code Intelligence Language Engine for the Jade language.
...
Module Usage:
    from cile_jade import scan
    mtime = os.stat("bar.jade")[stat.ST_MTIME]
    content = open("bar.jade", "r").read()
    scan(content, "bar.jade", mtime=mtime)
"""
...
log = logging.getLogger("cile.jade")
...
def scan_buf(buf, mtime=None, lang="Jade"):
    """Scan the given JadeBuffer return an ElementTree (conforming
    to the CIX schema) giving a summary of its code elements.
    
    @param buf {JadeBuffer} is the Jade buffer to scan
    ...
    # Dev Note:
    # This is where you process the Jade content and add CIX elements
    ...
    #   Use the "codeintel html -b <example-Jade-file>" command as
    #   a debugging tool.
    # - "buf.accessor.text" is the whole content of the file. If you have
    #   a separate tokenizer/scanner tool for Jade content, you may
    #   want to use it.
