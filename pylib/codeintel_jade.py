...
lang = "Jade"
log = logging.getLogger("codeintel.jade")
#log.setLevel(logging.DEBUG)


# These keywords are copied from "Jade-mainlex.udl" - be sure to keep both
# of them in sync.
keywords = [
    # Keywords
        "a",
...
# lexers. You must define a Lexer class for lang Jade. If Komodo's
# scintilla lexer for Jade is UDL-based, then this is simply:
#
#   from codeintel2.udl import UDLLexer
#   class JadeLexer(UDLLexer):
#       lang = lang
#
# Otherwise (the lexer for Jade is one of Komodo's existing C++ lexers
...
#           self._lexer = SilverCity.find_lexer_module_by_id(ScintillaConstants.SCLEX_JADE)
#           self._keyword_lists = [
...
# Jade content and returns a description of the code in that file.
# See "cile_jade.py" for more details.
#
# The CILE Driver is a class that calls this CILE. If Jade is
...
class JadeCILEDriver(UDLCILEDriver):
    lang = lang

    def scan_purelang(self, buf):
        import cile_jade
        return cile_jade.scan_buf(buf)
...
def register(mgr):
    ...
    mgr.set_lang_info(
        lang,
        silvercity_lexer=JadeLexer(),
        buf_class=JadeBuffer,
        langintel_class=JadeLangIntel,
        import_handler_class=None,
        cile_driver_class=JadeCILEDriver,
